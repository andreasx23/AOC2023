...
namespace AOC2024.Day20
{
    public class Day20Part1
    {
        ...
        {
            PriorityQueue<(int x, int y, int steps, HashSet<(int x, int y)> visisted, bool hasCheated, bool isCheating), int> queue = new();

            queue.Enqueue((x, y, 0, new HashSet<(int x, int y)>() { (x, y) }, false, false), ManhattenDistance(x, y, targetX, targetY));
            ...
            while (queue.Count > 0)
            {
                ...
                if (!current.hasCheated)
                {
                    foreach (var item in PossibleDirections(current.x, current.y, _grid, true))
                    {
                        ...
                        {
                            if (current.isCheating && _grid[item.x][item.y] == '.')
                            {
                                var newVisted = new HashSet<(int x, int y)>(current.visisted) { item };
                                queue.Enqueue((item.x, item.y, current.steps + 1, newVisted, true, false), ManhattenDistance(item.x, item.y, targetX, targetY));
                            }
                            else if (!current.isCheating)
                            {
                                var newVisted = new HashSet<(int x, int y)>(current.visisted) { item };
                                queue.Enqueue((item.x, item.y, current.steps + 1, newVisted, false, true), ManhattenDistance(item.x, item.y, targetX, targetY));
                            }
                        }
                    }
                }
                ...
                if (!current.isCheating)
                {
                    foreach (var item in PossibleDirections(current.x, current.y, _grid, false))
                    {
                        ...
                        {
                            var newVisted = new HashSet<(int x, int y)>(current.visisted) { item };
                            queue.Enqueue((item.x, item.y, current.steps + 1, newVisted, current.hasCheated, current.isCheating), ManhattenDistance(item.x, item.y, targetX, targetY));
                        }
                    }
                }
            }
